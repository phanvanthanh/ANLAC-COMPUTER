    ...
    <tbody>                       
        ...
        @foreach($donViTinhs as $donViTinh)
            ...
            <tr class="tr-hover tr-small">
                ...
                <td class="@if($donViTinh['state']==1) {{'text-primary'}} @else {{'text-danger'}} @endif">      
                    {{$donViTinh['ten_don_vi_tinh']}}
                </td>
                <td class="@if($donViTinh['state']==1) {{'text-primary'}} @else {{'text-danger'}} @endif">
                    <label class="">@if($donViTinh['state']==1) {{'Đang hoạt động'}} @else {{'Ngừng hoạt động'}} @endif</label>
                </td>
                <td class="text-center">
                    <button class="btn btn-vnpt" href="#" data-toggle="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="notificationDropdown">
                            <a class="dropdown-item preview-item">
                                <p class="mb-0 font-weight-normal float-left text-primary btn-sua" data="{{$donViTinh['id']}}"><b><i class="icon-wrench"></i> Sửa</b>
                                </p>
                            </a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item preview-item">
                                <p class="mb-0 font-weight-normal float-left text-danger btn-xoa" data="{{$donViTinh['id']}}"><b><i class="icon-basket "></i> Xóa</b>
                                </p>
                            </a>                                 
                        </div>
                    </button>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        jQuery('.btn-sua').on('click',function(){            
            ...
            getById(_token, id, "{{ route('don-vi-tinh-single') }}", ".frm-cap-nhat"); // gọi sự kiện lấy dữ liệu theo id
            $('#modal-cap-nhat').modal('show'); // bật form sửa     
        });

        /*Sự kiện bấm nút cập nhật*/
        jQuery('.btn-cap-nhat').on('click',function(){            
            capNhat(_token, $("form#frm-cap-nhat"), "{{ route('cap-nhat-don-vi-tinh') }}", "{{ route('danh-sach-don-vi-tinh') }}", '.load-danh-sach'); // bật form sửa     
        ...
        jQuery('.btn-xoa').on('click',function(){      
            ...
            if (result) {
                xoa(_token, id, "{{ route('xoa-don-vi-tinh') }}", "{{ route('danh-sach-don-vi-tinh') }}", '.load-danh-sach');  
            }
        });
